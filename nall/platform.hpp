...
#include <sys/stat.h>
  ...
  namespace nall {
    ...
    inline auto recv(int socket, void* buffer, size_t length, int flags) -> intmax_t { // Non-portable `ssize_t` replaced with `intmax_t` by MT.
      return ::recv(socket, (char*)buffer, length, flags);
    }

    inline auto send(int socket, const void* buffer, size_t length, int flags) -> intmax_t { // Non-portable `ssize_t` replaced with `intmax_t` by MT.
