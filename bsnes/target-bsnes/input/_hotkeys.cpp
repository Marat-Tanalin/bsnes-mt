...
auto InputManager::bindHotkeys() -> void {
	...
	hotkeys.append(InputHotkey("Rewind").onPress([&] {
		if (!emulator->loaded() || program.fastForwarding) {
			return;
		}
	...
	hotkeys.append(InputHotkey("Increment State Slot").onPress([&] {
		if (++stateSlot > bma::quickStatesNumber) {
			stateSlot = 1;
		}
	...
	hotkeys.append(InputHotkey("Fast Forward").onPress([] {
		if (!emulator->loaded() || program.rewinding) {
			return;
		}
	...
	hotkeys.append(InputHotkey("Increase HD Mode 7").onPress([] {
		...
		if (index >= enhancementSettings.mode7Scale.itemCount()) {
			return;
		}
	...
	for (auto& hotkey : hotkeys) {
		...
		for (uint index : range(BindingLimit)) {
			hotkey.assignments[index] = assignments(index);
		}
...
auto InputManager::pollHotkeys() -> void {
	if (Application::modal() || !program.focused()) {
		return;
	}
	...
	for (auto& hotkey : hotkeys) {
		...
		if (hotkey.state == 0 && state == 1 && hotkey.press) {
			hotkey.press();
		}

		if (hotkey.state == 1 && state == 0 && hotkey.release) {
			hotkey.release();
		}
