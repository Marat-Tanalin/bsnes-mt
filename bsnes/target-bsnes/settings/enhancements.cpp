...
auto EnhancementSettings::create() -> void {
  ...
  string framesString = bms::get("Common.FramesFew").data(); // MT.
  char space = ' ';

  runAheadLabel.setText(bms::get("Settings.Enhancements.RunAhead").data()).setFont(Font().setBold()); // "Run-Ahead"
  runAhead0.setText(bms::get("Common.Disabled").data()).onActivate([&] { // "Disabled"
    settings.emulator.runAhead.frames = 0;
  });
  runAhead1.setText({bms::get("Settings.Enhancements.RunAhead.One").data(), space, bms::get("Common.Frame").data()}).onActivate([&] { // "One Frame"
    settings.emulator.runAhead.frames = 1;
  });
  runAhead2.setText({bms::get("Settings.Enhancements.RunAhead.Two").data(), space, framesString}).onActivate([&] { // "Two Frames"
    settings.emulator.runAhead.frames = 2;
  });
  runAhead3.setText({bms::get("Settings.Enhancements.RunAhead.Three").data(), space, framesString}).onActivate([&] { // "Three Frames"
    settings.emulator.runAhead.frames = 3;
  });
  runAhead4.setText({bms::get("Settings.Enhancements.RunAhead.Four").data(), space, framesString}).onActivate([&] { // "Four Frames"
  ...
  ppuLabel.setText({"PPU (", bms::get("Settings.Enhancements.Video").data(), ')'}).setFont(Font().setBold()); // "PPU (video)"
  fastPPU.setText(bms::get("Common.FastMode").data()).setChecked(settings.emulator.hack.ppu.fast).onToggle([&] { // "Fast mode"
  ...
  deinterlace.setText(bms::get("Settings.Enhancements.Ppu.Deinterlace").data()).setChecked(settings.emulator.hack.ppu.deinterlace).onToggle([&] { // "Deinterlace"
  ...
  noSpriteLimit.setText(bms::get("Settings.Enhancements.Ppu.NoSpriteLimit").data()).setChecked(settings.emulator.hack.ppu.noSpriteLimit).onToggle([&] { // "No sprite limit"
  ...
  mode7Perspective.setText(bms::get("Settings.Enhancements.hdMode7.PerspectiveCorrection").data()).setChecked(settings.emulator.hack.ppu.mode7.perspective).onToggle([&] { // "Perspective correction"
    settings.emulator.hack.ppu.mode7.perspective = mode7Perspective.checked();
    emulator->configure("Hacks/PPU/Mode7/Perspective", settings.emulator.hack.ppu.mode7.perspective);
  });
  mode7Supersample.setText(bms::get("Settings.Enhancements.hdMode7.Supersampling").data()).setChecked(settings.emulator.hack.ppu.mode7.supersample).onToggle([&] {// "Supersampling"
    settings.emulator.hack.ppu.mode7.supersample = mode7Supersample.checked();
    emulator->configure("Hacks/PPU/Mode7/Supersample", settings.emulator.hack.ppu.mode7.supersample);
  });
  mode7Mosaic.setText(bms::get("Settings.Enhancements.hdMode7.HdSdMosaic").data()).setChecked(settings.emulator.hack.ppu.mode7.mosaic).onToggle([&] { // "HD->SD Mosaic"
  ...
  fastDSP.setText(bms::get("Common.FastMode").data()).setChecked(settings.emulator.hack.dsp.fast).onToggle([&] { // "Fast mode"
    settings.emulator.hack.dsp.fast = fastDSP.checked();
    emulator->configure("Hacks/DSP/Fast", settings.emulator.hack.dsp.fast);
  });
  cubicInterpolation.setText(bms::get("Settings.Enhancements.Dsp.CubicInterpolation").data()).setChecked(settings.emulator.hack.dsp.cubic).onToggle([&] { // "Cubic interpolation"
  ...
  coprocessorDelayedSyncOption.setText(bms::get("Common.FastMode").data()).setChecked(settings.emulator.hack.coprocessor.delayedSync).onToggle([&] { // "Fast mode"
    settings.emulator.hack.coprocessor.delayedSync = coprocessorDelayedSyncOption.checked();
  });
  coprocessorPreferHLEOption.setText(bms::get("Settings.Enhancements.Coprocessors.PreferHle").data()).setChecked(settings.emulator.hack.coprocessor.preferHLE).setToolTip( // "Prefer HLE"
