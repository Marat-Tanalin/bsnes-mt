struct Presentation : Window {
  ...
  enum : uint { RecentGames = 25, QuickStates = 25 }; // Changed from 9 to 25 by MT.
  ...
  MenuBar menuBar{this};
    Menu fileMenu{&menuBar}; // MT.
      MenuItem loadGame{&fileMenu};           // Replaced `systemMenu` with `fileMenu` by MT.
      Menu loadRecentGame{&fileMenu};         // Replaced `systemMenu` with `fileMenu` by MT.
      MenuItem unloadGame{&fileMenu};         // Replaced `systemMenu` with `fileMenu` by MT.
      MenuSeparator quitSeparator{&fileMenu}; // Replaced `systemMenu` with `fileMenu` by MT.
      MenuItem quit{&fileMenu};               // Replaced `systemMenu` with `fileMenu` by MT.
