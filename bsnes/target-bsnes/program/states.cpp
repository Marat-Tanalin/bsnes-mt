...
auto Program::hasState(string filename) -> bool {
	...
	return (bool)availableStates(type).find([&](auto& state) {
...
auto Program::loadStateData(string filename) -> vector<uint8_t> {
	...
	if (memory.size() < 3 * sizeof(uint)) {  //too small to be a valid state file
		return {};
	}

	if (memory::readl<sizeof(uint)>(memory.data()) != State::Signature) {  //wrong format or version
		return {};
	}
...
auto Program::saveState(string filename) -> bool {
	...
	if (screenshot.data) {
		...
		if (preview.width() != 256 || preview.height() != 240) {
			preview.scale(256, 240, true);
		}
	...
	if (gamePath().endsWith("/")) {
		...
		if (input.open(statePath())) {
			for (auto& file : input.file) {
				if (!file.name.endsWith(".bst") || file.name == location) {
					continue;
				}
